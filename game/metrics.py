...
class Metrics:
    ...
    @staticmethod
    def generate_vs_report(player1:str, player2:str, winner:str, points:int, total_points:int, depth:int):
        """Create the .csv file with the performance metrics."""
        ...
        with open(csv_filename, 'a', newline='') as f:
            writer = csv.writer(f)
            headers = ["player 1", "player 2", "winner", "winner points", "Total_points", "depth"]
            if not file_exists:
                writer.writerow(headers)
